...
;; URL parsing and building helpers.
...
fn parse_query(query){
	...
	while(i < len){
		...
		if(eq_pos >= 0){
			...
			setitem(result, urldecode(key), urldecode(value))
		} else if(str_len(pair) > 0){
			setitem(result, urldecode(pair), 1)
		}
