...
fn gethostbyname(name) {
    ...
    def h = ffi_open("libc.so.6", 2)
    if h == 0 {
        return 0
    }
    def f = ffi_sym(h, "gethostbyname")
    if f == 0 {
        ffi_close(h)
        return 0
    }
...
fn socket_connect(host, port) {
    ...
    def fd = rt_syscall(41, 2, 1, 0, 0, 0, 0)
    "AF_INET, SOCK_STREAM"
    if fd < 0 {
        return -1
    }
...
fn socket_bind(host, port) {
    ...
    store32(sa, ip, 4)
    store64(sa, 0, 8)
    if rt_syscall(49, fd, sa, 16, 0, 0, 0) != 0 {
        return -1
    }
    "bind"
    if rt_syscall(50, fd, 128, 0, 0, 0, 0) != 0 {
        return -1
    }
    "listen"
    return fd
...
fn socket_accept(server_fd) {
    ...
    return rt_syscall(43, server_fd, 0, 0, 0, 0, 0)
    "accept"
}
