...
fn http_get(host, port, path) {
	...
	def req = concat("GET ", concat(path, " HTTP/1.1\\n"))
	req = concat(req, concat("Host: ", concat(host, "\\n")))
...
fn http_post(host, port, path, data) {
	...
	def req = concat("POST ", concat(path, " HTTP/1.1\\n"))
	req = concat(req, concat("Host: ", concat(host, "\\n")))
	req = concat(req, concat("Content-Length: ", concat(itoa(len(data)), "\\n")))
	req = concat(req, "Connection: close\\n")
	req = concat(req, "\\n")
	req = concat(req, data)
	write_socket(fd, req)
...
fn http_put(host, port, path, data) {
	...
	def req = concat("PUT ", concat(path, " HTTP/1.1\\n"))
	req = concat(req, concat("Host: ", concat(host, "\\n")))
	req = concat(req, concat("Content-Length: ", concat(itoa(len(data)), "\\n")))
	req = concat(req, "Connection: close\\n\\n")
	req = concat(req, data)
...
fn http_delete(host, port, path) {
	...
	def req = concat("DELETE ", concat(path, " HTTP/1.1\\n"))
	req = concat(req, concat("Host: ", concat(host, "\\n")))
	req = concat(req, "Connection: close\\n\\n")
	write_socket(fd, req)
	def res = ""
